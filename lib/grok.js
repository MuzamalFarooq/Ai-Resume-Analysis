...
function getBaseUrl(apiKey) {
  if (process.env.GROK_BASE_URL || process.env.GROQ_BASE_URL || process.env.OPENAI_BASE_URL || process.env.AI_BASE_URL) {
    return (
  ...
  if (apiKey.startsWith("sk-") && !apiKey.startsWith("xai-")) {
    return "https://api.openai.com/v1";
  }
  return "https://api.x.ai/v1";
}

function getModelsToTry(apiKey) {
  const customModel = (process.env.GROK_MODEL || process.env.GROQ_MODEL || process.env.OPENAI_MODEL || "").trim();
...
export function getClient() {
  ...
  if (!aiClient || lastApiKey !== apiKey || lastBaseUrl !== baseURL) {
    aiClient = new OpenAI({
      apiKey,
      baseURL,
    });
    lastApiKey = apiKey;
    lastBaseUrl = baseURL;
    console.log(`[AI] Client initialized with baseURL: ${baseURL}`);
  }
  return aiClient;
}
...
export async function generateJSON(systemInstruction, userContent, temperature = 0.2, label = "generateJSON") {
  if (!isGrokConfigured()) {
    throw new Error("GROK_API_KEY missing");
  }
  ...
  const apiKey = getGrokApiKey();
  const models = getModelsToTry(apiKey);
  let lastError = null;

  for (const model of models) {
    try {
      console.log(`[AI] Sending request: ${label}`, { model });
      ...
      console.log(`[AI] Response OK: ${label} (used ${model})`);
      return parseJsonContent(content, label);
    } catch (err) {
      console.warn(`[AI] Model ${model} failed for ${label}:`, err.message || err);
      lastError = err;
  ...
  console.error(`[AI] All models failed for ${label}:`, lastError?.message || lastError);
  throw lastError;
}
